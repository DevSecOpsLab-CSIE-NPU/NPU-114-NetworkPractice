...
自動化建置 Docker 容器並匯出壓縮檔案的腳本。

**功能：**
- 清理舊的容器和映像
- 建置新的 Docker 映像（包含 STU_ID）
- 啟動容器進行測試
- 匯出 Docker 映像為 tar 檔案
- 使用 pbzip2 進行多核心並行壓縮
- 將壓縮檔案儲存到 `assets/` 目錄
...
```
assets/
└── http-basic-auth-server.tar.bz2  (壓縮後的 Docker 映像)
```
...
# 2. 載入映像
docker load -i assets/http-basic-auth-server.tar
...
# 或使用其他 port
docker run -d -p 8080:3128 --name http-auth-server http-basic-auth-server
```
